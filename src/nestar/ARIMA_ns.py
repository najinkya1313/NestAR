...
class ARIMA_Nested_Sampler:
    ...
    def mean_fit_plot(self, compare=None):
        ...
        y_fit = ARIMA_fast(
            self.data,
            self.order,
            self.posterior_means[p + q],
            self.posterior_means[p + q + 1],
            self.posterior_means[0:p],
            self.posterior_means[p:p + q],
            self.posterior_means[p + q + 1:2 * p + q + 1],
            self.seed
        )
        ...
        if compare is not None:
            if compare == True:
                ...
                raise SyntaxError(
                    f"Invalid value {compare} for compare argument. "
                    "compare should be == True, False, or None"
                )
        else:
            plt.plot(y_fit, label=f'ARIMA {self.order} model')
    ...
    def outsample_forecast(self, overall_time, overall_data, num_forecast, upper_index=None,
        ...
        **kwargs
        ...
        fig, results = forecaster.outsample_forecast(
            ...
            n_samples=n_samples,
            seed=seed if seed is not None else self.seed,
            plot_nested=plot_nested,
            plot_climatology=plot_climatology,
            plot_persistence=plot_persistence,
            ax=ax,
            **kwargs
        )
